...
class Obd2Plugin {
  ...
  Future<int> getParamsFromJSON(String jsonString, {int lastIndex = 0, int requestCode = 4}) async {
    ...
    int index = 0;
    if (stm.isEmpty) {
      throw Exception("OBD 형식 제대로 안지킴");
    }
    index = lastIndex;
        ...
        if (!string.contains('>')) {
          ...
          if (this.onResponse != null) {
            if (commandMode == Mode.parameter) {
              ...
              if (validResponse.contains(runningService["unit"].toString().toUpperCase()) ||
                  validResponse.contains(runningService["unit"].toString().toLowerCase())) {
              ...
              dtcCodesResponse += _getDtcsFrom(validResponse,
                  limit: "7F ${lastetCommand.contains(" ") ? lastetCommand.split(" ")[0] : lastetCommand.toString()}", command: lastetCommand);
              ...
              this.onResponse!(
                  lastetCommand, response.replaceAll("\n", "").replaceAll("\r", "").replaceAll(">", "").replaceAll("SEARCHING...", ""), requestCode);
              sendDTCToResponse = false;
              commandMode = Mode.at;
              requestCode = 999999999999999999;
              lastetCommand = "";
